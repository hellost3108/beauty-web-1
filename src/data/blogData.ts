export const blogPosts = [
    {
        id: 1,
        title: "BNBG – Gương Mặt Mới Trong Làng Làm Đẹp Hàn Quốc",
        excerpt: "BNBG là một trong những thương hiệu làm đẹp Hàn Quốc mà Melalogy tuyển chọn mang về. Cùng điểm qua vì sao cái tên này đáng để bạn khám phá.",
        author: "Đội Ngũ Melalogy",
        date: "13 Tháng 8, 2026",
        category: "Thương Hiệu",
        image: "/assets/mask-recovery-green-hero.png",
        imageAlt: "Mặt nạ hydrogel xanh lá trong bộ sưu tập chăm sóc da Melalogy",
        readingTime: "4 phút đọc",
        content: `
            <p>Trong hành trình tìm kiếm những thương hiệu làm đẹp chất lượng từ Hàn Quốc để mang đến cho khách hàng, BNBG là một trong những cái tên Melalogy đã chọn lựa và giới thiệu tới cộng đồng.</p>

            <h3>Vì Sao Đáng Chú Ý</h3>
            <p>Thị trường mỹ phẩm Hàn Quốc vốn nổi tiếng với tốc độ đổi mới nhanh và sự chỉn chu trong từng khâu phát triển sản phẩm. BNBG là một phần của làn sóng thương hiệu trẻ, mang tinh thần đó, hướng đến những khách hàng yêu thích trải nghiệm làm đẹp mới mẻ nhưng vẫn chú trọng chất lượng.</p>

            <h3>Tìm Hiểu Thêm</h3>
            <p>Melalogy sẽ tiếp tục cập nhật thêm thông tin chi tiết về các dòng sản phẩm của BNBG trong thời gian tới. Hãy theo dõi để không bỏ lỡ những sản phẩm mới nhất.</p>
        `
    },
    {
        id: 2,
        title: "Chăm Sóc Da Đúng Cách Cho Người Mới Bắt Đầu: Quy Trình 5 Bước Cơ Bản",
        excerpt: "Không cần 10 bước phức tạp. Đây là quy trình chăm sóc da cơ bản, dễ áp dụng cho mọi loại da, giúp bạn xây nền tảng vững chắc trước khi thêm bất kỳ sản phẩm đặc trị nào.",
        author: "Minh Anh",
        date: "08 Tháng 8, 2026",
        category: "Chăm Sóc Da",
        image: "/assets/melalogy-blog-routine-2026.png",
        imageAlt: "Bộ sản phẩm chăm sóc da năm bước trên nền đá màu kem",
        readingTime: "6 phút đọc",
        content: `
            <p>Giữa hàng trăm sản phẩm chăm sóc da trên thị trường, rất dễ bị cuốn vào việc mua sắm theo cảm tính mà quên mất điều quan trọng nhất: một quy trình chăm sóc da đúng thứ tự, đúng nhu cầu. Trước khi nghĩ đến serum đặc trị hay mặt nạ chuyên sâu, hãy chắc chắn bạn đã nắm vững 5 bước nền tảng dưới đây.</p>

            <h3>1. Làm Sạch Da Hai Lần</h3>
            <p>Bước làm sạch kép — bắt đầu bằng dầu tẩy trang hoặc sáp tẩy trang để loại bỏ lớp kem chống nắng, bụi bẩn, sau đó dùng sữa rửa mặt dịu nhẹ để làm sạch sâu — giúp da thực sự "thở" mà không bị khô căng.</p>

            <h3>2. Cân Bằng Với Toner</h3>
            <p>Một lớp toner không cồn giúp cân bằng độ pH cho da sau khi rửa mặt, đồng thời chuẩn bị bề mặt da để hấp thụ dưỡng chất tốt hơn ở các bước tiếp theo.</p>

            <h3>3. Dưỡng Ẩm Với Serum</h3>
            <p>Serum chứa Hyaluronic Acid hoặc Niacinamide là lựa chọn an toàn cho hầu hết mọi loại da, giúp cấp ẩm sâu và cải thiện kết cấu da theo thời gian.</p>

            <h3>4. Khoá Ẩm Với Kem Dưỡng</h3>
            <p>Kem dưỡng ẩm không chỉ làm mềm da mà còn tạo lớp màng bảo vệ, giữ cho các dưỡng chất ở bước trước không bốc hơi.</p>

            <h3>5. Chống Nắng Mỗi Sáng — Không Ngoại Lệ</h3>
            <p>Đây là bước bị bỏ qua nhiều nhất nhưng lại quan trọng nhất. Một lớp kem chống nắng phổ rộng SPF 30 trở lên nên là bước cuối cùng, không thể thiếu, trong quy trình buổi sáng của bạn.</p>
        `
    },
    {
        id: 3,
        title: "Wonjin – Tinh Hoa Chăm Sóc Da Chuẩn Phòng Khám Hàn Quốc",
        excerpt: "Wonjin là thương hiệu chăm sóc da được nhiều tín đồ làm đẹp Hàn Quốc tin dùng, nay đã có mặt trong bộ sưu tập của Melalogy.",
        author: "Đội Ngũ Melalogy",
        date: "03 Tháng 8, 2026",
        category: "Thương Hiệu",
        image: "/assets/skincare-mask-application.jpg",
        imageAlt: "Người mẫu đang sử dụng mặt nạ chăm sóc da",
        readingTime: "4 phút đọc",
        content: `
            <p>Wonjin là một thương hiệu quen thuộc với những ai theo dõi xu hướng skincare Hàn Quốc, gắn liền với triết lý chăm sóc da bài bản, chú trọng từng bước trong quy trình dưỡng da.</p>

            <h3>Triết Lý Thương Hiệu</h3>
            <p>Điểm thu hút của Wonjin nằm ở cách tiếp cận chăm sóc da nghiêm túc, tập trung vào việc phục hồi và cân bằng làn da như một quy trình liền mạch, thay vì chỉ dừng lại ở một vài sản phẩm rời rạc.</p>

            <h3>Có Gì Tại Melalogy</h3>
            <p>Melalogy đang giới thiệu Wonjin đến khách hàng Việt Nam như một phần trong danh mục các thương hiệu Hàn Quốc được tuyển chọn kỹ lưỡng. Ghé thăm chuyên mục Shop để khám phá thêm.</p>
        `
    },
    {
        id: 4,
        title: "Cách Chọn Son Môi Hợp Tông Da: Bí Quyết Từ Chuyên Gia",
        excerpt: "Xác định tông da lạnh, ấm hay trung tính chỉ trong 1 phút, và tìm ra bảng màu son môi tôn da nhất — hướng dẫn chọn son chuẩn không cần thử tại quầy.",
        author: "Ngọc Trần",
        date: "29 Tháng 7, 2026",
        category: "Trang Điểm",
        image: "/assets/melalogy-blog-makeup-2026.png",
        imageAlt: "Phụ nữ Việt Nam trang điểm môi với sắc son hồng đất",
        readingTime: "5 phút đọc",
        content: `
            <p>Bạn đã bao giờ mua một thỏi son vì thấy đẹp trên người khác, nhưng khi tô lên môi mình lại thấy không hợp? Bí quyết không nằm ở màu sắc đang hot, mà ở việc xác định đúng tông da của bạn.</p>

            <h3>Xác Định Tông Da Trong 1 Phút</h3>
            <p>Nhìn vào mạch máu ở cổ tay dưới ánh sáng tự nhiên: nếu mạch máu ánh xanh hoặc tím, bạn thuộc tông da lạnh; nếu ánh xanh lá hoặc vàng, bạn thuộc tông da ấm; nếu khó phân biệt, bạn thuộc tông da trung tính.</p>

            <h3>Tông Da Lạnh Nên Chọn Gì?</h3>
            <p>Các sắc đỏ ánh xanh (blue-red), hồng phấn lạnh, hoặc màu mận (berry) sẽ tôn da và giúp răng trông trắng sáng hơn.</p>

            <h3>Tông Da Ấm Nên Chọn Gì?</h3>
            <p>Sắc cam đất, đỏ gạch, hoặc hồng đào (coral) sẽ giúp làn da trông rạng rỡ, khoẻ khoắn tự nhiên.</p>

            <h3>Mẹo Nhỏ Khi Không Chắc Chắn</h3>
            <p>Nếu vẫn phân vân, hãy ưu tiên các gam màu nude hồng hoặc đỏ mận trung tính — đây là những "màu quốc dân" phù hợp với đa số tông da.</p>
        `
    },
    {
        id: 5,
        title: "Angel Liquid – Bí Quyết Dưỡng Da Trong Suốt Kiểu Hàn",
        excerpt: "Angel Liquid là sản phẩm quen thuộc với những tín đồ theo đuổi làn da căng bóng kiểu Hàn Quốc, nay có mặt trong danh mục của Melalogy.",
        author: "Đội Ngũ Melalogy",
        date: "24 Tháng 7, 2026",
        category: "Thương Hiệu",
        image: "/assets/3.jpg",
        imageAlt: "Kết cấu serum trong suốt với bọt khí cấp ẩm",
        readingTime: "4 phút đọc",
        content: `
            <p>Angel Liquid là cái tên được nhiều tín đồ làm đẹp Hàn Quốc nhắc đến khi nói về hành trình theo đuổi làn da căng bóng, mịn màng theo phong cách "glass skin".</p>

            <h3>Vì Sao Được Yêu Thích</h3>
            <p>Được biết đến như một bước dưỡng ẩm quan trọng trong quy trình skincare nhiều lớp kiểu Hàn, Angel Liquid gắn liền với trải nghiệm chăm sóc da nhẹ nhàng nhưng hiệu quả mà nhiều người tìm kiếm.</p>

            <h3>Có Tại Melalogy</h3>
            <p>Melalogy hiện đang giới thiệu Angel Liquid đến khách hàng như một phần trong bộ sưu tập các sản phẩm làm đẹp Hàn Quốc được tuyển chọn. Thông tin chi tiết về sản phẩm sẽ được cập nhật sớm.</p>
        `
    },
    {
        id: 6,
        title: "Dưỡng Ẩm Mùa Hanh Khô: 4 Bước Giữ Làn Da Căng Mịn",
        excerpt: "Da căng, bong tróc, dễ kích ứng khi trời hanh khô? Đây là 4 điều chỉnh đơn giản trong quy trình dưỡng da giúp bạn giữ được làn da mềm mịn quanh năm.",
        author: "Thu Hà",
        date: "18 Tháng 7, 2026",
        category: "Chăm Sóc Da",
        image: "/assets/mask-hydrating-blue.png",
        imageAlt: "Mặt nạ hydrogel cấp ẩm màu xanh của Melalogy",
        readingTime: "5 phút đọc",
        content: `
            <p>Khi độ ẩm không khí giảm, làn da mất nước nhanh hơn bình thường, dễ dẫn đến tình trạng căng rát, bong tróc, thậm chí kích ứng. Tin vui là bạn không cần thay đổi toàn bộ quy trình dưỡng da — chỉ cần điều chỉnh đúng 4 điểm sau.</p>

            <h3>1. Đổi Sữa Rửa Mặt Sang Dạng Dịu Nhẹ Hơn</h3>
            <p>Sữa rửa mặt tạo bọt nhiều thường chứa chất tẩy mạnh, dễ lấy đi lớp dầu tự nhiên vốn đã ít ỏi trong mùa hanh khô. Hãy chuyển sang dạng sữa hoặc gel không tạo bọt, pH dịu nhẹ.</p>

            <h3>2. Ưu Tiên Serum Cấp Ẩm Nhiều Lớp</h3>
            <p>Thay vì chỉ dùng một loại serum, hãy thử "layering" — thoa Hyaluronic Acid trên da còn ẩm, sau đó là một lớp serum khoá ẩm chứa Ceramide hoặc Squalane.</p>

            <h3>3. Chọn Kem Dưỡng Kết Cấu Đặc Hơn</h3>
            <p>Mùa hanh khô là lúc bạn có thể "nâng cấp" kem dưỡng ẩm sang kết cấu đặc, giàu dưỡng chất hơn so với mùa nóng ẩm.</p>

            <h3>4. Đừng Quên Dưỡng Ẩm Body Và Môi</h3>
            <p>Không chỉ da mặt, toàn thân và đôi môi cũng cần được chăm sóc kỹ hơn trong thời tiết hanh khô.</p>
        `
    },
    {
        id: 7,
        title: "APerire – Thương Hiệu Làm Đẹp Đáng Khám Phá",
        excerpt: "APerire là cái tên mới trong danh mục thương hiệu mà Melalogy giới thiệu, hứa hẹn mang đến trải nghiệm làm đẹp đáng để khám phá.",
        author: "Đội Ngũ Melalogy",
        date: "12 Tháng 7, 2026",
        category: "Thương Hiệu",
        image: "/assets/shop-hero.png",
        imageAlt: "Bộ sưu tập mặt nạ hydrogel nhiều màu của Melalogy",
        readingTime: "3 phút đọc",
        content: `
            <p>APerire là một trong những thương hiệu mới nhất được Melalogy lựa chọn để giới thiệu tới cộng đồng yêu làm đẹp tại Việt Nam.</p>

            <h3>Định Hướng Thương Hiệu</h3>
            <p>Cũng như các thương hiệu khác trong danh mục tuyển chọn của Melalogy, APerire được đưa vào với tiêu chí chất lượng và trải nghiệm sử dụng thực tế lên hàng đầu.</p>

            <h3>Cập Nhật Sắp Tới</h3>
            <p>Thông tin chi tiết về các dòng sản phẩm của APerire sẽ được Melalogy cập nhật trong thời gian tới. Hãy theo dõi để không bỏ lỡ.</p>
        `
    },
    {
        id: 8,
        title: "Trang Điểm Tự Nhiên Đi Làm: 5 Bước Nhanh Gọn Mỗi Sáng",
        excerpt: "Không cần dậy sớm cả tiếng đồng hồ. Đây là 5 bước trang điểm tự nhiên, tôn da, phù hợp đi làm hàng ngày mà bạn có thể hoàn thành trong chưa đầy 10 phút.",
        author: "Ngọc Trần",
        date: "05 Tháng 7, 2026",
        category: "Trang Điểm",
        image: "/assets/skincare-face-lifestyle.jpg",
        imageAlt: "Chân dung làn da tự nhiên dưới ánh sáng ban ngày",
        readingTime: "5 phút đọc",
        content: `
            <p>Trang điểm đi làm không cần cầu kỳ. Mục tiêu là giúp bạn trông tươi tắn, chỉn chu nhưng vẫn tự nhiên như không hề trang điểm.</p>

            <h3>1. Kem Lót Dưỡng Ẩm</h3>
            <p>Bắt đầu với một lớp kem lót mỏng nhẹ, ưu tiên loại có thành phần dưỡng ẩm để lớp nền phía sau bám đều suốt ngày dài.</p>

            <h3>2. Kem Nền Mỏng Nhẹ</h3>
            <p>Chọn kem nền dạng BB cream hoặc tinted moisturizer thay vì lớp nền phủ dày.</p>

            <h3>3. Che Khuyết Điểm Có Chọn Lọc</h3>
            <p>Chỉ cần che ở vùng quầng thâm mắt và các đốm mụn rõ rệt, thay vì phủ che toàn mặt.</p>

            <h3>4. Tán Đều Má Hồng</h3>
            <p>Một chút má hồng tông đào hoặc hồng nhạt được tán ngay trên gò má sẽ giúp gương mặt trông tươi tỉnh.</p>

            <h3>5. Điểm Nhấn Với Son</h3>
            <p>Kết thúc bằng một lớp son dưỡng có màu hoặc son tint tự nhiên.</p>
        `
    },
    {
        id: 9,
        title: "Cách Chọn Kem Chống Nắng Phù Hợp Với Từng Loại Da",
        excerpt: "Kem chống nắng vật lý hay hoá học? Dạng gel hay dạng kem? Hướng dẫn chọn đúng loại kem chống nắng cho da dầu, da khô và da nhạy cảm.",
        author: "Thu Hà",
        date: "28 Tháng 6, 2026",
        category: "Chăm Sóc Da",
        image: "/assets/5.jpg",
        imageAlt: "Minh họa lớp màng bảo vệ da và dưỡng chất",
        readingTime: "6 phút đọc",
        content: `
            <p>Chống nắng là bước không thể thiếu trong bất kỳ quy trình chăm sóc da nào, nhưng không phải loại kem chống nắng nào cũng phù hợp với mọi loại da.</p>

            <h3>Kem Chống Nắng Vật Lý Hay Hoá Học?</h3>
            <p>Kem chống nắng vật lý (khoáng chất) tạo lớp màng chắn trên bề mặt da, phù hợp với da nhạy cảm, dễ kích ứng. Kem chống nắng hoá học thẩm thấu vào da và thường có kết cấu mỏng nhẹ hơn, phù hợp để trang điểm bên trên.</p>

            <h3>Da Dầu Nên Chọn Gì?</h3>
            <p>Ưu tiên kết cấu dạng gel hoặc dạng sữa mỏng nhẹ, không chứa dầu, có khả năng kiềm dầu nhẹ để tránh cảm giác bóng nhờn suốt ngày dài.</p>

            <h3>Da Khô Nên Chọn Gì?</h3>
            <p>Chọn kem chống nắng có thêm thành phần dưỡng ẩm như Hyaluronic Acid hoặc Glycerin để tránh cảm giác căng khô sau khi thoa.</p>

            <h3>Da Nhạy Cảm Nên Chọn Gì?</h3>
            <p>Ưu tiên các sản phẩm không hương liệu, không cồn, và đã được kiểm nghiệm da liễu (dermatologically tested) để giảm nguy cơ kích ứng.</p>
        `
    }
];
